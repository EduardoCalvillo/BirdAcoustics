...
for(i in 1:qty){
    ...
    x <- c(runif(paquete,0,50),runif(paquete,50,100),runif(paquete,100,150),runif(paquete,150,200)) #Numeros aleatorios pero con limites marcados por nivel de prescencia
    # matrix(c(y,x), nrow = nrow)
    z <- runif(nrow,0,200) #Numeros completamente aleatorios 
    ...
    print(cor(z,y, method = "spearman")) #Esto da valores muy aleatorios. Muchas veces cercanos a 0, lo cual indica que NO hay correlación
}
